...
def recurse_bbg_data(fetch_kernel, dates, ticker_bbg):
    for date in dates:
        try:
...
def recurse_bdh(ticker_bbg, date_ref, fld, n=30, bcon=BCON):
    fetch = partial(fetch_bbg_data, fld=fld, bcon=bcon)
    dates = generate_date_sequence(date_ref, n)
    data = partial(recurse_bbg_data, fetch, dates)(ticker_bbg)
